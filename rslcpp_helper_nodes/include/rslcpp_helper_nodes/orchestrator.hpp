...
public:
  ...
  : Node("PlaybackOrchestratorHelperNode", options), abort_simulation_callback_(abort_callback)
  {
    ...
    sub_player_ready_ = this->create_subscription<std_msgs::msg::Bool>(
      "/player/player_ready", rclcpp::QoS(1).reliable().transient_local(),
      std::bind(&PlaybackOrchestrator::player_ready_callback, this, std::placeholders::_1));
...
private:
  ...
  void player_ready_callback(const std_msgs::msg::Bool::SharedPtr msg)
  {
    player_ready_ = msg->data;
  }
  ...
  void timer_callback()
  {
    ...
    if ((this->get_clock()->now() - start_time_).seconds() >= timeout) {
    ...
    switch (state_) {
      ...
      case State::START_PLAYER:
        if (!start_player_requested_) {
          if (!client_start_player_->wait_for_service(std::chrono::seconds(0))) {
            return;
          }
      ...
      case State::STOP_RECORDER:
        if (!stop_recorder_requested_) {
          if (!client_stop_recorder_->wait_for_service(std::chrono::seconds(0))) {
            return;
          }
...
private:
  State state_{State::WAIT_PLAYER_READY};
  ...
  rclcpp::Subscription<std_msgs::msg::Bool>::SharedPtr sub_player_ready_{};
